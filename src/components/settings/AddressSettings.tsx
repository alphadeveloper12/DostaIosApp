...
export default function AddressSettings() {
  ...
  const handleAddAddress = async () => {
    setLoading(true);
    try {
      const res = await axios.post(`${baseUrl}/addresses/`, newAddress, {
        headers: { Authorization: `Token ${authToken}` },
      });
  ...
  return (
    <div className="space-y-2">
      ...
      {/* New Address Form */}
      <section className="bg-neutral-white rounded-[16px] py-2 md:px-[16px] md:py-[24px] sm:p-8 shadow-sm">
        <h2 className="text-lg font-semibold mb-6 text-foreground">New address</h2>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {["label", "address_line_1", "address_line_2", "city"].map((field) => (
