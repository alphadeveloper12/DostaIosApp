...
interface BudgetSelectionProps {
  ...
  setSelectedBudget: React.Dispatch<React.SetStateAction<{ id: string | null; label: string | null; price_range: string | null }>>;
  handleGoBack: () => void;
  handleContinue: () => void;
}
...
const BudgetSelection: React.FC<BudgetSelectionProps> = ({
  ...
  const [budgetOptions, setBudgetOptions] = useState<{ id: string; label: string; price_range: string }[]>([]); // State to store the fetched budget options
  const [loading, setLoading] = useState<boolean>(true); // State for loading status
  const [error, setError] = useState<string | null>(null); // State for error message

  const baseUrl = import.meta.env.VITE_API_URL || "http://127.0.0.1:8000/api/catering/";
  ...
  useEffect(() => {
    const fetchBudgetOptions = async () => {
      try {
        const response = await axios.get(`${baseUrl}budget-options/`, {
          headers: {
  ...
  if (loading) {
    return <div>Loading budget options...</div>;
  }
  ...
  const handleBudgetSelection = (budget: { id: string; label: string; price_range: string }) => {
      ...
      <div className="ml-12">
        <div className="grid md:grid-cols-4 gap-6 max-w-5xl">
          {budgetOptions.map((budget) => (
            <Button
              key={budget.id}
              onClick={() => handleBudgetSelection({ id: budget.id, label: budget.label, price_range: budget.price_range })}
              style={{
                fontSize: "16px",
                height: "80px",
